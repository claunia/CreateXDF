...
namespace CreateXDF
{
    class MainClass
    {
        static readonly byte[] bpb = { 0xEB, 0x4F, 0x90, 0x49, 0x42, 0x4D, 0x20, 0x32, 0x30, 0x2E, 0x30, 0x00, 0x02, 0x01, 0x01, 0x00,
									   ...
									   0x73, 0x73, 0x69, 0x6F, 0x6E, 0x0A, 0x0A, 0xC0, 0x6E, 0xEF, 0xDD, 0x5F, 0x00, 0x00, 0x00, 0x00,
        ...
        static readonly byte[] minibpb = { ,
                                           ...
                                           0x69, 0x63, 0x61, 0x74, 0x69, 0x6F, 0x6E, 0x20, 0x70, 0x72, 0x6F, 0x68, 0x69, 0x62, 0x69, 0x74,
                                           0x65, 0x64, 0x20, 0x77, 0x69, 0x74, 0x68, 0x6F, 0x75, 0x74, 0x20, 0x70, 0x65, 0x72, 0x6D, 0x69,
                                           0x73, 0x73, 0x69, 0x6F, 0x6E, 0x0A, 0x0A, 0xC0, 0xDB, 0xCD, 0xDA, 0x55, 0x00, 0x00, 0x00, 0x00,
                                           0x4F, 0x53, 0x2F, 0x32, 0x20, 0x21, 0x21, 0x20, 0x53, 0x59, 0x53, 0x30, 0x31, 0x34, 0x37, 0x35,
                                           0x0D, 0x0A, 0x4F, 0x53, 0x2F, 0x32, 0x20, 0x21, 0x21, 0x20, 0x53, 0x59, 0x53, 0x30, 0x32, 0x30,
                                           0x32, 0x37, 0x0D, 0x0A, 0x00, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x50, 0x61, 0x74, 0x65, 0x6E,
                                           0x74, 0x28, 0x73, 0x29, 0x20, 0x50, 0x65, 0x6E, 0x64, 0x69, 0x6E, 0x67, 0x0D, 0x0A, 0x0D, 0x0A,
        ...
        static readonly byte[] miniFatEntry = { 0xF9, 0xFF, 0xFF, 0x03, 0x40, 0x00, 0xFF, 0x2F };

        const ushort rootEntries = 224;

        const byte formatByte = 0xF6;

        const string miniMessage = "This is a XDF disk created with CreateXDF.\n\r" +
            "You need to use IBM's XDF.COM driver to access its contents.";
        ...
        public static void Main(string[] args)
        {
			...
			// Write miniBPB
            fs.Write(minibpb, 0, 512);
            fs.Seek(-512, SeekOrigin.Current);
            fs.Seek(0x26, SeekOrigin.Current);
			fs.WriteByte(0x29);
			fs.Write(serial, 0, 4);
			tmp = Encoding.ASCII.GetBytes(volname.ToUpper());
			fs.Write(tmp, 0, tmp.Length >= 11 ? 11 : tmp.Length);
            fs.Seek(512 - 0x36, SeekOrigin.Current);

			// Write miniFAT
			byte[] miniFat = new byte[512];
            Array.Copy(miniFat, 0, miniFatEntry, 0, miniFatEntry.Length);
            fs.Write(miniFat, 0, 512);
            fs.Write(miniFat, 0, 512);
            ...
            for (int i = 0; i < 11; i++)
                dirEntry[i] = 0x20;
			tmp = Encoding.ASCII.GetBytes(volname.ToUpper());
            Array.Copy(tmp, 0, dirEntry, 0, tmp.Length >= 11 ? 11 : tmp.Length);
            // Attributes
            dirEntry[11] = 8;
            // Case information
            dirEntry[12] = 0;
            ...
            dirEntry[13] = 0;
            // Creation time
            Array.Copy(ctime_b, 0, dirEntry, 14, 2);
			// Creation date
            Array.Copy(cdate_b, 0, dirEntry, 16, 2);
			...
			dirEntry[21] = 0;
			// Last written time
			Array.Copy(ctime_b, 0, dirEntry, 22, 2);
			// Last written date
			Array.Copy(cdate_b, 0, dirEntry, 24, 2);
			// Starting cluster
			dirEntry[26] = 0;
			dirEntry[27] = 0;
			// Size
            ...
            Array.Copy(dirEntry, 0, rootDir, 0, 32);

            // Write mini root
            byte[] miniDir = new byte[512];
            Array.Copy(dirEntry, 0, miniDir, 0, 32);
            tmp = Encoding.ASCII.GetBytes("README  TXT");
            Array.Copy(tmp, 0, dirEntry, 0, 11);
            dirEntry[11] = 1;
			dirEntry[26] = 2;
			int messageLen = Encoding.ASCII.GetBytes(miniMessage).Length;
            tmp = BitConverter.GetBytes((uint)(messageLen < 512 ? messageLen : 512));
			Array.Copy(tmp, 0, dirEntry, 28, 4);
            Array.Copy(dirEntry, 0, miniDir, 32, 32);
            fs.Write(miniDir, 0, 512);

            // Write message file
            tmp = Encoding.ASCII.GetBytes(miniMessage);
            fs.Write(tmp, 0, tmp.Length >= 512 ? 512 : tmp.Length);

            // Write root
            fs.Seek(spt * 512, SeekOrigin.Begin);
            ...
            fs.Seek((fatSectors + 1) * 512 + 0x13C, SeekOrigin.Begin);
            fs.Write(checksum, 0, 4);
